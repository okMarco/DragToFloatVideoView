...
public abstract class VideoControl extends FrameLayout implements View.OnClickListener {
	...
	protected TextView tvChangePosition, tvProgress, tvDuration;
	...
	public abstract void onPlayStateChange();
	...
	private static final class UpdateProgressTask implements Runnable {
		...
		public void run() {
			if (mControlWeakReference != null && mControlWeakReference.get() != null) {
				mControlWeakReference.get().updateProgress(VideoPlayer.getInstance(mControlWeakReference.get().getContext()).getCurrentPosition());
			}
		}
	}
