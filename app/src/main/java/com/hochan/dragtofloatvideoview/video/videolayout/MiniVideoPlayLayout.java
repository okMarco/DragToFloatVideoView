...
import com.hochan.dragtofloatvideoview.video.videocontrol.MiniVideoControl;
...
public class MiniVideoPlayLayout extends VideoPlayLayout {
	...
	public MiniVideoPlayLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			mVideoPlayBinding.videoTextureView.setOutlineProvider(new RoundCornerViewOutlineProvider(10));
			mVideoPlayBinding.videoTextureView.setClipToOutline(true);
	...
	public void onPlayStateChange() {
		...
		if (VideoPlayer.getInstance(getContext()).getCurrentState() == NativeVideoPlayer.STATE_COMPLETED) {
			if (mOnDragListener != null) {
